...
export function BetSlip() {
  ...
  useEffect(() => {
    if (items.length === 1 && !isMobileOpen) {
      ...
      {items.length > 0 && (
        <div className="p-4 bg-gray-50 border-t border-gray-200 flex-shrink-0">
          ...
          <button
            ...
            disabled={placing}
            className="w-full bg-ton-blue hover:bg-ton-darkblue text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-blue-500/20 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2 active:scale-95"
          >
            {placing ? (
              <>
    ...
    <>
      ...
      {items.length > 0 && (
        <div className="hidden lg:block fixed right-6 top-24 w-80 z-40 animate-slide-in-right">
          <div className="bg-white rounded-2xl border border-gray-200 shadow-xl overflow-hidden flex flex-col max-h-[calc(100vh-8rem)]">
            <BetSlipContent />
          </div>
        </div>
      )}
